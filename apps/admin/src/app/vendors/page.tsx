...
export default function AdminVendorsPage() {
  ...
  const fetchItems = async () => {
    ...
    try {
      ...
      const res  = await fetch(`${API}/admin/vendors?${params}`, { headers: hdrs() });
  ...
  const action = async (id: string, act: string) => {
    ...
    try {
      const res  = await fetch(`${API}/admin/vendors/${id}`, { 
        method: 'PUT', 
        headers: hdrs(),
        body: JSON.stringify({ action: act, ...(act === 'suspend' && { reason: suspendReason, details: suspendDetails }) })
      });
      const data = await res.json();
        ...
        <nav className="flex-1 px-3 py-4 space-y-1">
          {NAV.map(n => (
            <Link key={n.href} href={n.href}
              className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors ${n.href === '/vendors' ? 'bg-red-500 text-white' : 'text-gray-400 hover:text-white hover:bg-gray-800'}`}>
      ...
      <div className="flex-1 flex flex-col max-w-screen">
        <header className="sticky top-0 z-30 bg-white border-b border-gray-200 px-4 sm:px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div><h1 className="text-xl font-black text-gray-900">Vendors</h1><p className="text-xs text-gray-500">Manage platform vendors</p></div>
          </div>
        ...
        <main className="flex-1 p-4 sm:p-6 space-y-5">
          ...
          <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
            ...
            items.length === 0 ? <div className="p-16 text-center text-gray-500">No vendors found</div> :
                  ...
                  <tr key={item.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex gap-2">
                        ...
                        {item.status === 'approved' && <button onClick={() => { setSuspendTargetId(item.id); setSuspendReason('Policy violation'); setSuspendDetails(''); setSuspendModalOpen(true); }} disabled={acting===item.id} className="bg-gray-100 hover:bg-gray-200 text-gray-700 text-xs font-bold px-3 py-1.5 rounded-lg disabled:opacity-60">Suspend</button>}
                        {item.status === 'suspended' && <button onClick={()=>action(item.id,'unsuspend')} disabled={acting===item.id} className="bg-green-50 hover:bg-green-100 text-green-700 text-xs font-bold px-3 py-1.5 rounded-lg disabled:opacity-60">Activate</button>}
                      </div>
                    </td>
                  </tr>
                ))}
          ...
          {suspendModalOpen && (
            <>
              <div className='fixed inset-0 z-70 bg-black/50' onClick={() => setSuspendModalOpen(false)}></div>
              <div className='fixed z-80 flex items-center justify-center bg-white rounded-lg top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-lg mx-4'>
                <div className='flex flex-col w-full' onClick={(e) => e.stopPropagation()}>
                  <h1 className='text-lg font-bold text-gray-900 p-6'>Select reason for suspension</h1>
                  <div className='w-full h-px bg-gray-600'></div>
                  <div className='flex flex-col gap-3 p-6'>
                    <label className='flex items-center gap-3'>
                      <input type='radio' name='reason' value='violation' checked={suspendReason === 'Policy violation'} onChange={() => setSuspendReason('Policy violation')} className='w-4 h-4 text-red-500 border-gray-300 focus:ring-red-500' />
                      <span className='text-gray-900'>Policy Violation</span>
                    </label>
                    <label className='flex items-center gap-3'>
                      <input type='radio' name='reason' value='inactivity' checked={suspendReason === 'inactivity'} onChange={() => setSuspendReason('inactivity')} className='w-4 h-4 text-red-500 border-gray-300 focus:ring-red-500' />
                      <span className='text-gray-900'>Inactivity</span>
                    </label>
                    <label className='flex items-center gap-3'>
                      <input type='radio' name='reason' value='other' checked={suspendReason === 'other'} onChange={() => setSuspendReason('other')} className='w-4 h-4 text-red-500 border-gray-300 focus:ring-red-500' />
                      <span className='text-gray-900'>Other (please specify)</span>
                    </label>
                    <textarea value={suspendDetails} onChange={(e) => setSuspendDetails(e.target.value)} placeholder='Enter reason here...' className='w-full p-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-red-500' />
                  </div>
                  <div className='flex gap-3 p-6'>
                    <button onClick={() => setSuspendModalOpen(false)} className='px-4 py-2 bg-gray-100 text-gray-900 rounded-lg hover:bg-gray-200'>Cancel</button>
                    <button onClick={() => { setSuspendModalOpen(false); if (suspendTargetId) action(suspendTargetId, 'suspend'); }} className='px-4 py-2 bg-red-500 text-white rounded-lg hover:bg-red-600'>Confirm</button>
                  </div>
                </div>
              </div>
            </>

...
export default function AdminRidersPage() {
  ...
  const fetchItems = async () => {
    ...
    try {
      ...
      const res  = await fetch(`${API}/admin/riders?${params}`, { headers: hdrs() });
  ...
  const action = async (id: string, act: string) => {
    setActing(id);
    try {
      const res  = await fetch(`${API}/admin/riders/${id}/${act}`, { method: 'PUT', headers: hdrs() });
      const data = await res.json();
        ...
        <nav className="flex-1 px-3 py-4 space-y-1">
          {NAV.map(n => (
            <Link key={n.href} href={n.href}
              className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors ${n.href === '/riders' ? 'bg-red-500 text-white' : 'text-gray-400 hover:text-white hover:bg-gray-800'}`}>
      ...
      <div className="flex-1 flex flex-col">
        <header className="sticky top-0 z-30 bg-white border-b border-gray-200 px-4 sm:px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div><h1 className="text-xl font-black text-gray-900">Riders</h1><p className="text-xs text-gray-500">Manage platform riders</p></div>
          </div>
        ...
        <main className="flex-1 p-4 sm:p-6 space-y-5">
          ...
          <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
            ...
            items.length === 0 ? <div className="p-16 text-center text-gray-500">No riders found</div> :

...
export default function AdminCouponsPage() {
  ...
  const createCoupon = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (form.target_min_spend)
        body.target_min_spend = parseFloat(form.target_min_spend);
      ...
      if (data.success) {
        toast.success(
          "Coupon created! " +
            (form.target_min_spend ? "Qualifying users notified." : ""),
        );
        setShowForm(false);
        setForm({ ...EMPTY_FORM });
        fetchCoupons();
  ...
  const toggleCoupon = async (id: string, current: number) => {
    try {
      ...
      setCoupons((cs) =>
        cs.map((c) => (c.id === id ? { ...c, is_active: current ? 0 : 1 } : c)),
      );
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      {/* Header */}
      <div className="bg-linear-to-r from-indigo-600 via-purple-600 to-pink-600 py-8">
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 py-8 space-y-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          {[
            ...
            {
              label: "Inactive",
              value: coupons.length - activeCount,
              icon: Clock,
              color: "orange",
            },
          ].map((s) => (
            ...
            >
              <div
                className={`w-8 h-8 bg-${s.color}-100 rounded-xl flex items-center justify-center mx-auto mb-2`}
              >
            ...
            <form onSubmit={createCoupon} className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  <label className="label">
                    Value *{" "}
                    {form.type === "fixed" ? "(₦ amount off)" : "(% discount)"}
                  </label>
                  <input
                    type="number"
                    min="0"
                    ...
                    className="input"
                    placeholder={
                      form.type === "fixed" ? "e.g. 1000" : "e.g. 10"
                    }
                    value={form.value}
                    onChange={(e) =>
                ...
                <div>
                  ...
                  <input
                    ...
                    min="0"
                    className="input"
                    placeholder="e.g. 5000 (or 0 for no min)"
                ...
                <div className="sm:col-span-2">
                  ...
                  <input
                    className="input"
                    placeholder="e.g. Eid promotion, Returning customer reward…"
              ...
              <div className="bg-blue-50 rounded-xl p-4 border border-blue-100">
                ...
                <p className="text-xs text-blue-600 mb-3">
                  Optionally, auto-notify all customers who have spent above a
                  threshold. Leave blank to skip.
                </p>
                <div>
                  ...
                  <input
                    ...
                    placeholder="e.g. 30060 → notify customers who spent ≥ ₦30,000"
                    value={form.target_min_spend}
                    onChange={(e) =>
                      setForm((f) => ({
                        ...f,
                        target_min_spend: e.target.value,
                      }))
                    }
                  />
                </div>
            ...
            {filtered.map((c) => (
              ...
              >
                <div className="flex items-center gap-4 px-5 py-4">
                  {/* Coupon code badge */}
                  <div className="bg-linear-to-r from-purple-500 to-pink-500 rounded-xl px-4 py-2 text-white text-center min-w-30 shrink-0">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex flex-wrap gap-3 mt-1 text-xs text-gray-500">
                      {c.min_order > 0 && (
                        <span>
                          Min order: ₦{Number(c.min_order).toLocaleString()}
                        </span>
                      )}
                      <span>
        ...
        <div className="bg-amber-50 rounded-2xl p-5 border border-amber-100">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs text-amber-700">
            <div className="space-y-1.5">
              <p>
                <strong>Fixed:</strong> Subtracts a fixed naira amount from the
                order total
              </p>
              <p>
                <strong>Percent:</strong> Deducts a percentage of the order
                total
              </p>
              <p>
                <strong>Referral:</strong> Auto-generated 0.5% coupons for
                referred users
              </p>
            </div>
            <div className="space-y-1.5">
              <p>
                <strong>Auto-notify:</strong> Enter a min spend threshold to
                automatically push the coupon to qualifying loyal customers
              </p>
              <p>
                <strong>Max Uses:</strong> Set to 0 for unlimited usage
                (market-wide promos)
              </p>
              <p>
                <strong>Code:</strong> Leave blank to auto-generate a unique
                code
              </p>
            </div>
          </div>

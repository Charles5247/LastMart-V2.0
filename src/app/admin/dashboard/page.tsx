...
export default function AdminDashboard() {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      {/* Header */}
      <div className="bg-linear-to-r from-gray-800 via-gray-900 to-gray-800 py-10">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          {[
            ...
            <div key={s.label} className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
              <div className="flex items-start justify-between mb-4">
                <div className={`w-12 h-12 bg-linear-to-br ${s.color} rounded-2xl flex items-center justify-center shadow-lg`}>
        ...
        {(stats?.pendingVendors > 0) && (
          <div className="bg-yellow-50 border border-yellow-200 rounded-2xl p-4 mb-6 flex items-center justify-between">
            ...
            <Link href="/admin/vendors" className="bg-yellow-500 text-white px-4 py-2 rounded-xl text-sm font-bold hover:bg-yellow-600 transition-colors">
              Review Now
            </Link>
          </div>
        )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <Link href="/admin/vendors" className="text-sm text-orange-500 hover:text-orange-600 font-medium flex items-center gap-1">View All <ArrowRight className="w-3 h-3" /></Link>
            </div>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <Link href="/admin/orders" className="text-sm text-orange-500 hover:text-orange-600 font-medium flex items-center gap-1">View All <ArrowRight className="w-3 h-3" /></Link>
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
          {stats?.pendingKyc > 0 && (
            <div className="bg-purple-50 border border-purple-200 rounded-2xl p-4 flex items-center justify-between">
              ...
              <Link href="/admin/kyc" className="bg-purple-500 text-white px-3 py-1.5 rounded-xl text-xs font-bold hover:bg-purple-600">Review</Link>
            </div>
          )}
          {stats?.pendingProductVerifications > 0 && (
            <div className="bg-teal-50 border border-teal-200 rounded-2xl p-4 flex items-center justify-between">
              ...
              <Link href="/admin/product-verification" className="bg-teal-500 text-white px-3 py-1.5 rounded-xl text-xs font-bold hover:bg-teal-600">Review</Link>
            </div>
          )}
          {stats?.rankingApps?.length > 0 && (
            <div className="bg-yellow-50 border border-yellow-200 rounded-2xl p-4 flex items-center justify-between">
              ...
              <Link href="/admin/rankings" className="bg-yellow-500 text-white px-3 py-1.5 rounded-xl text-xs font-bold hover:bg-yellow-600">Review</Link>
            </div>
          )}
        </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          {[
            ...
            <div key={s.label} className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
              <div className={`w-10 h-10 bg-linear-to-br ${s.color} rounded-xl flex items-center justify-center shadow mb-3`}>
            ...
            {analytics?.recentKyc?.length > 0 && (
              <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <Link href="/admin/kyc" className="text-sm text-orange-500 font-medium flex items-center gap-1">View All <ArrowRight className="w-3 h-3" /></Link>
                </div>
                <div className="space-y-2">
                  {analytics.recentKyc.map((k: any) => (
                    <div key={k.id} className="flex items-center gap-3 p-3 bg-gray-50 rounded-xl">
                      <Shield className="w-8 h-8 text-purple-200 shrink-0" />
                      <div className="flex-1 min-w-0">
            ...
            {analytics?.rankingApps?.length > 0 && (
              <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <Link href="/admin/rankings" className="text-sm text-orange-500 font-medium flex items-center gap-1">View All <ArrowRight className="w-3 h-3" /></Link>
                </div>
                <div className="space-y-2">
                  {analytics.rankingApps.map((r: any) => (
                    <div key={r.id} className="flex items-center gap-3 p-3 bg-gray-50 rounded-xl">
                      <Crown className="w-8 h-8 text-yellow-200 shrink-0" />
                      <div className="flex-1 min-w-0">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          {[
            { href: '/admin/vendors', icon: Store, label: 'Manage Vendors', badge: stats?.pendingVendors || 0, color: 'from-purple-400 to-purple-600' },
            { href: '/admin/customers', icon: Users, label: 'Manage Customers', badge: stats?.suspendedUsers || 0, color: 'from-blue-400 to-blue-600' },
            { href: '/admin/kyc', icon: Shield, label: 'KYC Reviews', badge: stats?.pendingKyc || 0, color: 'from-indigo-400 to-indigo-600' },
            { href: '/admin/rankings', icon: Crown, label: 'Rankings & Ads', badge: analytics?.rankingApps?.length || 0, color: 'from-yellow-400 to-yellow-600' },
            { href: '/admin/product-verification', icon: Package, label: 'Product Vetting', badge: stats?.pendingProductVerifications || 0, color: 'from-teal-400 to-teal-600' },
            { href: '/admin/disputes', icon: AlertTriangle, label: 'Disputes', badge: 0, color: 'from-red-400 to-red-600' },
            { href: '/admin/riders', icon: Users, label: 'Rider Management', badge: 0, color: 'from-orange-400 to-orange-600' },
            { href: '/admin/products', icon: Package, label: 'All Products', badge: 0, color: 'from-cyan-400 to-cyan-600' },
            { href: '/admin/orders', icon: ShoppingBag, label: 'All Orders', badge: 0, color: 'from-orange-400 to-orange-600' },
            { href: '/admin/notifications', icon: Bell, label: 'Send Notifications', badge: 0, color: 'from-pink-400 to-pink-600' },
            { href: '/admin/analytics', icon: TrendingUp, label: 'Analytics', badge: 0, color: 'from-green-400 to-green-600' },
            { href: '/admin/coupons', icon: Tag, label: 'Coupons & Referrals', badge: 0, color: 'from-rose-400 to-rose-600' },
          ].map(link => (
            <Link key={link.href + link.label} href={link.href} className="bg-white rounded-2xl shadow-sm p-5 flex flex-col items-center gap-3 hover:shadow-md transition-shadow border border-gray-100 relative">
              <div className={`w-12 h-12 bg-linear-to-br ${link.color} rounded-2xl flex items-center justify-center shadow`}>

  ...
  return (
    <div className="overflow-x-auto">
      <div className="flex gap-1 mb-1">
        <div className="w-8" />
        {hours.map((h, i) => (
          <div key={i} className="w-4 text-center text-[9px] text-gray-400 shrink-0">{h}</div>
        ))}
      </div>
      {days.map((day, di) => (
        <div key={di} className="flex items-center gap-1 mb-1">
          <div className="w-8 text-[10px] text-gray-500 text-right pr-1 shrink-0">{day}</div>
          {Array.from({ length: 24 }, (_, hi) => {
            const val = data[di]?.[hi] || 0;
            return (
              <div
                key={hi}
                className={`w-4 h-4 rounded-sm shrink-0 ${getColor(val)} cursor-pointer transition-opacity hover:opacity-80`}
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
              {[
                ...
                <div key={m.label} className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                  <div className={`w-12 h-12 bg-linear-to-br ${m.color} rounded-2xl flex items-center justify-center mb-4 shadow-lg`}>

...
export default function VendorDashboard() {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      {/* Header */}
      <div className="bg-linear-to-r from-purple-600 via-pink-600 to-orange-500 py-10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            ...
            <div className="flex gap-3 flex-wrap">
              <Link href="/vendor/messages" className="flex items-center gap-2 bg-blue-400/30 backdrop-blur-sm text-white border border-blue-300/30 px-4 py-2 rounded-xl text-sm font-medium hover:bg-blue-400/40 transition-all">
                💬 Messages
              </Link>
              <Link href="/vendor/payouts" className="flex items-center gap-2 bg-green-400/30 backdrop-blur-sm text-white border border-green-300/30 px-4 py-2 rounded-xl text-sm font-medium hover:bg-green-400/40 transition-all">
                💰 Payouts
              </Link>
              <Link href="/vendor/promotions" className="flex items-center gap-2 bg-yellow-400/30 backdrop-blur-sm text-white border border-yellow-300/30 px-4 py-2 rounded-xl text-sm font-medium hover:bg-yellow-400/40 transition-all">
                🎯 Promotions
              </Link>
              <Link href="/vendor/subscription" className="flex items-center gap-2 bg-purple-400/30 backdrop-blur-sm text-white border border-purple-300/30 px-4 py-2 rounded-xl text-sm font-medium hover:bg-purple-400/40 transition-all">
                👑 Subscription
              </Link>
              <Link href="/vendor/products" className="flex items-center gap-2 bg-white/20 backdrop-blur-sm text-white border border-white/30 px-4 py-2 rounded-xl text-sm font-medium hover:bg-white/30 transition-all">
                <Plus className="w-4 h-4" /> Add Product
              </Link>
              <Link href="/vendor/analytics" className="flex items-center gap-2 bg-white text-orange-600 px-4 py-2 rounded-xl text-sm font-bold hover:bg-orange-50 transition-all shadow">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {(vendor as any)?.kyc_status !== 'approved' && (
          <div className="bg-purple-50 border border-purple-200 rounded-2xl p-4 mb-6 flex items-center justify-between gap-3">
            ...
            <Link href="/vendor/verification" className="bg-purple-500 text-white px-4 py-2 rounded-xl text-sm font-bold hover:bg-purple-600 transition-colors shrink-0">
              Verify Now
            </Link>
          </div>
        )}
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
          {[
            ...
            { href: '/vendor/ranking', label: 'Ranking & Ads', icon: '📈', color: 'bg-yellow-50 border-yellow-200 text-yellow-700' },
            { href: '/vendor/orders', label: 'Manage Orders', icon: '📦', color: 'bg-blue-50 border-blue-200 text-blue-700' },
            { href: '/vendor/products', label: 'Add Products', icon: '➕', color: 'bg-green-50 border-green-200 text-green-700' },
            { href: '/vendor/qr', label: 'QR & Share Link', icon: '📲', color: 'bg-teal-50 border-teal-200 text-teal-700' },
          ].map(l => (
        ...
        <div className="grid grid-cols-2 lg:grid-cols-5 gap-4 mb-8">
          {[
            ...
            <div key={s.label} className="bg-white rounded-2xl shadow-sm p-5 border border-gray-100">
              <div className={`w-10 h-10 bg-linear-to-br ${s.color} rounded-xl flex items-center justify-center mb-3`}>
              ...
              <div className="space-y-3">
                {analytics.ordersByStatus.map((s: any) => (
                  <div key={s.status} className="flex items-center gap-3">
                    <span className={`text-xs font-bold px-2 py-1 rounded-full ${getStatusColor(s.status)} shrink-0 w-24 text-center`}>{s.status}</span>
                    <div className="flex-1 bg-gray-100 rounded-full h-2 overflow-hidden">
                      <div className="bg-linear-to-r from-orange-400 to-pink-500 h-full rounded-full" style={{ width: `${(s.count / stats.totalOrders) * 100}%` }} />
                    </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mt-8">
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <Link href="/vendor/products" className="text-sm text-orange-500 hover:text-orange-600 font-medium flex items-center gap-1">Manage <ArrowRight className="w-3 h-3" /></Link>
            </div>
            {analytics?.topProducts?.length > 0 ? (
              ...
              <div className="text-center py-8">
                ...
                <Link href="/vendor/products" className="text-orange-500 text-sm font-medium mt-2 block">Add your first product</Link>
              </div>
            )}
          </div>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <Link href="/vendor/orders" className="text-sm text-orange-500 hover:text-orange-600 font-medium flex items-center gap-1">View All <ArrowRight className="w-3 h-3" /></Link>
            </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4 mt-8">
          {[
            { href: '/vendor/products', icon: Package, label: 'Products', color: 'text-purple-600 bg-purple-50' },
            { href: '/vendor/inventory', icon: AlertTriangle, label: 'Inventory', color: 'text-red-600 bg-red-50' },
            { href: '/vendor/orders', icon: ShoppingBag, label: 'Orders', color: 'text-blue-600 bg-blue-50' },
            { href: '/vendor/analytics', icon: TrendingUp, label: 'Analytics', color: 'text-green-600 bg-green-50' },
            { href: '/vendor/ads', icon: Megaphone, label: 'Advertise', color: 'text-orange-600 bg-orange-50' },
            { href: '/vendor/qr', icon: QrCode, label: 'QR & Share', color: 'text-teal-600 bg-teal-50' },
          ].map(link => (

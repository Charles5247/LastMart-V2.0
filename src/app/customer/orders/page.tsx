                  ...
                  <div className="grid grid-cols-1 gap-2 mb-4">
                    {order.items.map((item: any) => (
                      <div key={item.id} className="flex items-center gap-3 bg-gray-50 rounded-xl px-3 py-2">
                        <img src={item.product_image || `https://picsum.photos/seed/${item.product_id}/60/60`} alt={item.product_name} className="w-10 h-10 rounded-lg object-cover shrink-0" />
                    ...
                    <div className="space-y-3">
                      {order.tracking_updates.map((update: any, i: number) => (
                        <div key={i} className="flex items-start gap-3">
                          <div className={`w-3 h-3 rounded-full mt-0.5 shrink-0 ${i === order.tracking_updates.length - 1 ? 'bg-orange-500' : 'bg-gray-300'}`} />
                          <div>

...
export default function CustomerDashboard() {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      {/* Header */}
      <div className="bg-linear-to-r from-orange-500 to-pink-600 py-10">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
          {[
            ...
            { href: '/customer/orders', label: 'My Orders', icon: '📦', color: 'bg-blue-50 border-blue-200 text-blue-700' },
            { href: '/customer/wishlist', label: 'Wishlist', icon: '❤️', color: 'bg-red-50 border-red-200 text-red-700' },
            { href: '/customer/referrals', label: 'Referrals', icon: '🎁', color: 'bg-pink-50 border-pink-200 text-pink-700' },
            { href: '/customer/disputes', label: 'Disputes', icon: '⚖️', color: 'bg-yellow-50 border-yellow-200 text-yellow-700' },
            { href: '/customer/returns', label: 'Returns', icon: '↩️', color: 'bg-green-50 border-green-200 text-green-700' },
                ...
                {cart?.items?.length > 0 ? (
                  <>
                    <div className="space-y-3 mb-6">
                      {cart.items.map((item: any) => (
                        <div key={item.id} className="flex items-center gap-4 bg-gray-50 rounded-2xl p-4">
                          <img src={item.product?.images?.[0] || `https://picsum.photos/seed/${item.product_id}/80/80`} alt={item.product?.name} className="w-16 h-16 rounded-xl object-cover shrink-0" />
                    ...
                    <div className="bg-orange-50 rounded-2xl p-6">
                      ...
                      <button onClick={() => setShowCheckout(true)} className="w-full mt-4 bg-linear-to-r from-orange-500 to-pink-600 text-white py-4 rounded-2xl font-bold hover:opacity-90 transition-opacity shadow-lg">
                        Proceed to Checkout
                      </button>
                    </div>
                  </>
              ...
              <div>
                {orders.length > 0 ? (
                  <div className="space-y-4">
                    ...
                    <Link href="/customer/orders" className="block text-center text-orange-500 font-medium py-3 hover:text-orange-600">View All Orders →</Link>
                  </div>
                ) : (
                  <div className="text-center py-16">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          {[
            { href: '/customer/orders', icon: Package, label: 'All Orders', color: 'text-blue-600 bg-blue-50' },
            { href: '/customer/saved', icon: Heart, label: 'Saved Vendors', color: 'text-red-600 bg-red-50' },
            { href: '/customer/settings', icon: Settings, label: 'Account Settings', color: 'text-gray-600 bg-gray-50' },
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6 space-y-4">
              ...
              <button onClick={checkout} disabled={checkingOut} className="w-full bg-linear-to-r from-orange-500 to-pink-600 text-white py-4 rounded-2xl font-bold hover:opacity-90 disabled:opacity-50 flex items-center justify-center gap-2">

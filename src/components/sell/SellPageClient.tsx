...
import {
  ArrowRight, CheckCircle, Store, Package, TrendingUp,
  Zap, Shield, Headphones, BarChart2, Smartphone, Globe,
  ChevronDown, ChevronUp, Star, Users, DollarSign, Clock,
  Truck, Tag, CreditCard, Bell, Layers, Award, MapPin, Check
} from 'lucide-react';

/* ─── Metrics ────────────────────────────────────────────────────────────── */
const METRICS = [
  { value: '50,000+',  label: 'Active Customers',   icon: Users },
  { value: '1,200+',   label: 'Verified Vendors',   icon: Store },
...
/* ─── Feature grid ───────────────────────────────────────────────────────── */
const FEATURES = [
  { icon: BarChart2,  title: 'Real-time Analytics',    desc: 'Track sales, revenue, and customer insights with beautiful dashboards.' },
  { icon: Bell,       title: 'Instant Order Alerts',    desc: 'Get notified instantly via SMS, WhatsApp, and in-app when new orders arrive.' },
  { icon: Smartphone, title: 'Mobile-first Dashboard',  desc: 'Manage your store from your phone. Accept orders even without a laptop.' },
  { icon: CreditCard, title: 'Fast Payouts',            desc: 'Funds are settled within 24–48 hours directly to your bank account.' },
  { icon: Globe,      title: 'City-wide Reach',         desc: 'Show up in search results for customers across your city and nearby areas.' },
  { icon: Layers,     title: 'Bulk Product Upload',     desc: 'Upload hundreds of products at once via CSV. Save hours of manual work.' },
  { icon: Shield,     title: 'Dispute Protection',      desc: 'Vendor-friendly dispute resolution. We protect legitimate sellers too.' },
  { icon: Tag,        title: 'Promotions & Coupons',    desc: 'Run sales, create coupon codes, and boost visibility with paid ads.' },
];
...
function RevenueCalculator() {
  ...
  return (
    <div className="panel p-6 sm:p-8">
      <h3 className="text-xl font-black text-gray-800 mb-2">Revenue Calculator</h3>
      <p className="text-sm text-gray-500 mb-6">Estimate your monthly earnings on LastMart</p>

      <div className="space-y-6">
        {/* Products slider */}
        <div>
          <div className="flex items-center justify-between mb-2">
            <label className="text-sm font-semibold text-gray-700">Number of products listed</label>
            <span className="text-sm font-black text-orange-600">{products}</span>
          </div>
          <input
        ...
        <div>
          <div className="flex items-center justify-between mb-2">
            <label className="text-sm font-semibold text-gray-700">Average product price</label>
            <span className="text-sm font-black text-orange-600">{fmt(avgPrice)}</span>
          </div>
          <input
        ...
        <div>
          <div className="flex items-center justify-between mb-2">
            <label className="text-sm font-semibold text-gray-700">Estimated orders per day</label>
            <span className="text-sm font-black text-orange-600">{ordersPerDay} orders</span>
          </div>
          <input
            ...
            className="w-full accent-orange-500"
          />
          <div className="flex justify-between text-xs text-gray-400 mt-1"><span>1/day</span><span>100/day</span></div>
        </div>
      </div>
      ...
      <div className="mt-6 grid grid-cols-2 gap-3">
        <div className="bg-orange-50 rounded-xl p-4 text-center">
          ...
          <p className="text-xl font-black text-orange-600">{fmt(monthlyRevenue)}</p>
        </div>
        <div className="bg-green-50 rounded-xl p-4 text-center">
          <p className="text-xs text-gray-500 mb-1">Net After Fees (Growth)</p>
          <p className="text-xl font-black text-green-600">{fmt(Math.max(0, netMonthly))}</p>
        </div>
      </div>

      <p className="text-xs text-gray-400 mt-3 text-center">
        * Estimate based on Growth plan (3% fee + ₦5,000/mo). Actual results vary.
      </p>
  ...
  return (
    ...
    <main>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            ...
            <div>
              ...
              <p className="text-gray-300 text-lg leading-relaxed mb-8 max-w-lg">
                ...
                <strong className="text-white">50,000+ active customers</strong> across Nigeria
                and start earning — with zero listing fees.
              </p>
              ...
              <ul className="space-y-2.5 mb-10">
                {[
                  ...
                  <li key={b} className="flex items-center gap-2.5 text-gray-300 text-sm">
              ...
              <p className="text-gray-500 text-xs mt-4">
                No credit card required · Cancel subscription anytime · Free Starter plan forever
              </p>
            </div>
            ...
            <div className="hidden lg:block">
              <div className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-3xl p-6 space-y-4">
                ...
                <div className="flex items-center gap-3 pb-4 border-b border-white/10">
                  ...
                  <div>
                    ...
                    <p className="text-gray-400 text-xs flex items-center gap-1">
                      <CheckCircle className="w-3 h-3 text-green-400" /> Verified Vendor · Lagos
                    </p>
                  </div>
                ...
                <div className="grid grid-cols-3 gap-3">
                  {[
                    { label: 'Today\'s Sales', value: '₦47,500', icon: DollarSign, color: 'text-green-400' },
                    { label: 'New Orders',     value: '12',       icon: Package,    color: 'text-blue-400'  },
                    { label: 'Customers',      value: '284',      icon: Users,      color: 'text-purple-400'},
                  ].map(s => (
                ...
                {/* Mock recent orders */}
                <div className="space-y-2">
                  <p className="text-gray-400 text-xs font-semibold uppercase tracking-wide">Recent Orders</p>
                  {[
                    { product: 'Samsung Galaxy A54',  price: '₦185,000', status: 'new',       statusColor: 'bg-blue-500/20 text-blue-400' },
                    { product: 'Wireless Earbuds Pro', price: '₦22,500',  status: 'confirmed', statusColor: 'bg-green-500/20 text-green-400' },
                    { product: 'Power Bank 20,000mAh', price: '₦8,500',   status: 'shipped',   statusColor: 'bg-yellow-500/20 text-yellow-400' },
                  ].map(o => (
                    <div key={o.product} className="flex items-center gap-3 bg-white/5 rounded-xl px-3 py-2.5">
                      <div className="w-8 h-8 bg-gray-700 rounded-lg flex items-center justify-center flex-shrink-0 text-xs">📦</div>
                      <div className="flex-1 min-w-0">
                        <p className="text-white text-xs font-medium truncate">{o.product}</p>
                        <p className="text-gray-400 text-xs">{o.price}</p>
                      </div>
                      <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${o.statusColor}`}>
                        {o.status}
                      </span>
                    </div>
                  ))}
                ...
                <div className="bg-white/5 rounded-2xl p-4">
                  ...
                  <p className="text-gray-400 text-xs mt-2 text-center">Monthly revenue trend ↗ +47%</p>
                </div>
              </div>
            </div>
          </div>
            ...
            {METRICS.map(({ value, label, icon: Icon }) => (
              <div key={label} className="flex items-center justify-center gap-3 px-4 py-3">
                ...
                <div className="text-center sm:text-left">
                  ...
                  <p className="text-xs text-orange-200 leading-none mt-0.5">{label}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <section id="how-it-works" className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 scroll-mt-20">
        <div className="text-center mb-12">
          ...
          <p className="text-gray-500 max-w-xl mx-auto">
            From registration to your first sale — our onboarding is designed to get you live as quickly as possible.
          </p>
        </div>
            ...
            <div key={step.step} className="relative">
              ...
              <div className="panel p-6 h-full hover:shadow-md transition-shadow">
                <div className="flex items-start gap-4 mb-4">
                  ...
                  <div className="text-5xl font-black text-gray-100 leading-none mt-1">{step.step}</div>
                </div>
                <h3 className="font-black text-gray-800 text-xl mb-2">{step.title}</h3>
      ...
      <section className="bg-gray-50 py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
            ...
            {/* Calculator */}
            <div>
              <span className="text-xs font-bold text-orange-500 uppercase tracking-widest">Earning Potential</span>
            ...
            {/* Testimonials */}
            <div>
              <span className="text-xs font-bold text-green-600 uppercase tracking-widest">Success Stories</span>
              <h2 className="text-2xl font-black text-gray-900 mt-2 mb-6">
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2.5">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-gray-500">{t.store} · {t.city}</p>
                        </div>
                      </div>
                      <div className="text-right">
                        <p className="text-sm font-black text-green-600">{t.revenue}</p>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className="text-center mb-12">
          <span className="text-xs font-bold text-orange-500 uppercase tracking-widest">Platform Features</span>
          <h2 className="text-3xl md:text-4xl font-black text-gray-900 mt-2 mb-3">
            Everything You Need to Succeed
          </h2>
          <p className="text-gray-500 max-w-xl mx-auto">
            A complete toolkit built specifically for Nigerian market sellers — not a generic global platform.
          </p>
        </div>
      ...
      <section className="bg-gray-50 py-16">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {PRICING.map(plan => (
              ...
              >
                ...
                <div className="mb-4">
                  <h3 className={`font-black text-xl mb-1 ${plan.highlight ? 'text-white' : 'text-gray-800'}`}>
                    {plan.name}
                  </h3>
                  <p className={`text-xs mb-4 ${plan.highlight ? 'text-orange-100' : 'text-gray-500'}`}>
                    {plan.description}
                  </p>
                  <div className="flex items-end gap-1">
                    ...
                    {plan.period && (
                      <span className={`text-sm pb-1 ${plan.highlight ? 'text-orange-200' : 'text-gray-400'}`}>
                        {plan.period}
                      </span>
                    )}
                  </div>
                ...
                <ul className="space-y-2.5 mb-6 flex-1">
                  {plan.features.map(f => (
                    <li key={f} className={`flex items-start gap-2 text-sm ${plan.highlight ? 'text-orange-50' : 'text-gray-600'}`}>
                      <CheckCircle className={`w-4 h-4 flex-shrink-0 mt-0.5 ${plan.highlight ? 'text-orange-200' : 'text-green-500'}`} />
                      {f}
                    </li>
                  ))}
                </ul>
          ...
          <p className="text-center text-sm text-gray-400 mt-8">
            All plans include buyer protection, SSL security, and access to the LastMart vendor community.
          </p>
        </div>
      </section>
      ...
      <section className="bg-gradient-to-r from-gray-900 to-gray-800 py-20">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <p className="text-gray-300 text-lg mb-8 leading-relaxed">
            Join Nigeria's fastest-growing marketplace. Start free, grow with confidence,
            and reach customers who are ready to buy.
          </p>

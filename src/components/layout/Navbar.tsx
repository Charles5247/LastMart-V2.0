      ...
      <div className="bg-orange-500">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center gap-3 h-[60px]">
            ...
            {/* Location pill */}
            <div ref={cityRef} className="relative hidden lg:block flex-shrink-0">
              <button
                onClick={() => setCityOpen(!cityOpen)}
                className="flex items-center gap-1.5 bg-orange-600/60 hover:bg-orange-600/80 text-white rounded-md px-3 py-1.5 text-sm transition-colors"
              >
                <MapPin className="w-3.5 h-3.5" />
                <span className="max-w-[80px] truncate font-medium">{location.city}</span>
                <ChevronDown className="w-3 h-3 opacity-70" />
              </button>
              {cityOpen && (
                <div className="absolute top-full left-0 mt-2 w-60 bg-white rounded-xl shadow-xl border border-gray-100 p-4 animate-slide-down z-50">
                  <p className="text-xs font-semibold text-gray-500 mb-2">Change delivery city</p>
                  <input
                    value={cityInput}
                    ...
                    autoFocus
                  />
                  <button onClick={handleCitySet} className="btn-primary w-full text-sm py-2">Apply</button>
                </div>
              )}
            </div>

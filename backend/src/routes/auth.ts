...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      db.prepare(
        `INSERT INTO users (id, name, email, password, role, city, phone, address, latitude, longitude) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      ).run(
        userId,
        name,
        email,
      ...
      if (userRole === "vendor") {
        ...
        db.prepare(
          `INSERT INTO vendors (id, user_id, store_name, description, category, city, latitude, longitude) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
        ).run(
          vendorId,
          userId,
          store_name,
      ...
      if (referral_code) {
        try {
          ...
          if (referrer && referrer.id !== userId) {
            ...
            const refId = uuidv4();
            db.prepare(
              `INSERT INTO coupons (id, code, type, value, description, max_uses, created_by) VALUES (?, ?, 'referral', 0.5, ?, 1, ?)`,
            ).run(
              couponId,
              couponCode,
            ...
            db.prepare(
              `INSERT INTO referrals (id, referrer_id, referred_id, coupon_id, status) VALUES (?, ?, ?, ?, 'pending')`,
          ...
          console.warn(
            "Referral processing error (non-fatal):",
            refErr.message,
          );
        }
      }
      ...
      const user = db
        .prepare(
          "SELECT id, name, email, role, city, created_at FROM users WHERE id = ?",
        )
        .get(userId) as any;
      const token = signToken({
        userId: user.id,
...
router.post("/forgot", resetLimiter, async (req: Request, res: Response) => {
  try {
    ...
    const resetLink = `${process.env.FRONTEND_URL || "http://localhost:3006"}/auth/reset?token=${token}&email=${encodeURIComponent(user.email)}`;
    await sendEmail({
      to: user.email,
...
router.post("/reset", resetLimiter, async (req: Request, res: Response) => {
  try {
    ...
    db.prepare(
      'UPDATE users SET password = ?, password_reset_token = NULL, password_reset_expires = NULL, updated_at = (datetime("now")) WHERE id = ?',
    ).run(hashed, user.id);
...
router.post("/admin-reset", async (req: Request, res: Response) => {
  try {
    ...
    if (!user_id && !email)
      return res
        .status(400)
        .json({ success: false, error: "user_id or email required" });

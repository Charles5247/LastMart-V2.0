...
router.get("/validate/:code", (req: Request, res: Response) => {
  ...
  if (!coupon)
    return res
      .status(404)
      .json({ success: false, error: "Coupon not found or inactive" });
  ...
  if (orderAmount > 0 && orderAmount < coupon.min_order) {
    return res
      ...
      .json({
        success: false,
        error: `Minimum order of ₦${coupon.min_order.toLocaleString()} required`,
      });
  }
...
router.delete("/:id", (req: Request, res: Response) => {
  ...
  const db = getDB();
  db.prepare(`UPDATE coupons SET is_active = 0 WHERE id = ?`).run(
    req.params.id,
  );
  return res.json({ success: true });
});
...
router.get("/referral/stats", (req: Request, res: Response) => {
  ...
  return res.json({
    ...
    data: {
      ...
      referral_url: me?.referral_code
        ? `${process.env.FRONTEND_URL || "http://localhost:3006"}/auth/register?ref=${me.referral_code}`
        : null,
...
router.post("/referral/register", (req: Request, res: Response) => {
  ...
  if (referrer.id === new_user_id)
    return res
      .status(400)
      .json({ success: false, error: "Cannot refer yourself" });
  ...
  `,
    ...
    couponId,
    couponCode,
    `Referral welcome discount from ${referrer.name}`,
    referrer.id,
  );
  ...
  return res.json({
    success: true,
    data: { coupon_code: couponCode, message: "Referral processed" },
  });
});
...
router.post("/vendor-referral", (req: Request, res: Response) => {
  ...
  if (!vendor_id || !user_id) {
    return res
      .status(400)
      .json({ success: false, error: "vendor_id and user_id required" });
  }

  // Verify share token
  const vendor = db
    .prepare(
      `SELECT id, store_name, user_id FROM vendors WHERE id = ? AND share_token = ?`,
    )
    .get(vendor_id, share_token) as any;
  if (!vendor)
    return res
      .status(404)
      .json({ success: false, error: "Invalid vendor share link" });
  ...
  if (alreadyGot)
    return res.json({
      success: true,
      skipped: true,
      message: "Already received coupon",
    });
  ...
  `,
    ...
    couponId,
    couponCode,
    `Vendor referral – ${vendor.store_name} (${vendor_id})`,
    vendor.user_id,
  );
